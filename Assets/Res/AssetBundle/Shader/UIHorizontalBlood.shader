Shader "Tang/UIHorizontalBlood" {
		...
		col.a = uv.x > _Fill ? 0 : col.a;
		col.a *= _Trans;

		float add = _Fill - uv.x < 0.05?((_Fill - uv.x)*46)*1.3:1;

		half4 color = (tex2D(_MainTex, uv)*tex2D(_BlendX, float2(no1.x,no2.x))*_Color + _TextureSampleAdd)*1.3*add;
